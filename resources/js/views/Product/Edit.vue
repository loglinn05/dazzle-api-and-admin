<template>
    <Layout>
        ...
        <FadeTransition>
            ...
            ></i>
            <div v-else class="w-fit grid gap-y-10">
                <FloatLabel>
                    <InputText
                        id="title"
                ...
                <Button
                    label="Edit product"
                    class="w-fit"
                    @click="gatherDataAndEditProduct"
                />
            </div>
        </FadeTransition>
    </Layout>
...
import { isObject, isBoolean, getFileExtension } from "../../helpers.js";
...
function gatherDataAndEditProduct() {
    ...
    for (const key in productWithoutImages) {
        ...
        } else if (isObject(productWithoutImages[key])) {
            productData.append(key, productWithoutImages[key].id);
        } else if (isBoolean(productWithoutImages[key])) {
